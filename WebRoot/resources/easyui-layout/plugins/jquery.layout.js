...
var _3f="expand"+_3b.substring(0,1).toUpperCase()+_3b.substring(1); //expandWest expandEast ..
if(!_3d[_3f]){ //判断是否包含panel
_3d[_3f]=_40(_3b);
...
var _43=_42();
if(!_a(_3d[_3f])){
_3d.center.panel("resize",_43.resizeC); //调整center 大小
}
...
function _42(){
var cc=$(_3a);
var _4a=_3d.center.panel("options");//center panel属性
var _4b=1; //panel折叠后高度或者宽度
if(_3b=="east"){
var _4c=p.panel("panel")._outerWidth();
var _4d=_4a.width+_4c-_4b; //center panel与右边panel宽度和 减去折叠宽度 预留折叠后panel条宽度
if(_3e.split||!_3e.border){
_4d++; //边框+1
}
// resizeC center panel重新设置宽度,expand点击折叠panel展开位置，expandP折叠panel条位置，collapse
...
var hh=_4a.height; //center panel高度
if(!_a(_3d.expandNorth)){
hh+=_4e+((_3e.split||!_3e.border)?1:0);
}
_3d.east.add(_3d.west).add(_3d.expandEast).add(_3d.expandWest).panel("resize",{top:_4b-1,height:hh,width:_3d.west.css('width')});
...
return $.data(jq[0],"layout").panels[_6a];
	...
	var re =jq.each(function(){
		_39(this,_6b);
	});
	// 隐藏 south expand和north expand
//	var northExpand = $('.panel.layout-expand.layout-expand-north.panel-htop');
//	var southExpand = $('.panel.layout-expand.layout-expand-south.panel-htop');
//	if(northExpand.length != 0){
//		northExpand.css('visibility','hidden');
//	}
//	if(southExpand.length != 0){
//		southExpand.css('visibility','hidden');
//	}
	return re;
...
$(function(){
	easyloader.load('layout', function(){// load the specified module
...
function positionBTN(){
	$('.layout_toolbar').remove();
	$('.panel.layout-panel.layout-panel-west.panel-htop.layout-split-west').each(function(){//west panel
		var centerPanel = $(this).parent().children('.panel.layout-panel.layout-panel-center.panel-htop');
		
		var _left = centerPanel.css('left');
		var _top = centerPanel.css('top');
		var _bottom = centerPanel.css('bottom');
		var _height = centerPanel.css('height');
		var ab_top = parseInt(_top)+parseInt(_height)/2;
		var ab_left = _left;
		var ele = $(this);
		var btn;
		if(parseInt(_left) == 0){
			btn = $("<p class='layout_toolbar_lr_close layout_toolbar' style='cursor:pointer;z-index:10;position:absolute;top:"+ab_top+";left:"+ab_left+"' )}></p>");
		}else{
			btn = $("<p class='layout_toolbar_lr_open layout_toolbar' style='cursor:pointer;z-index:10;position:absolute;top:"+ab_top+";left:"+ab_left+"' )}></p>");
		}
		
		btn.click(function(){
			if(parseInt(_left) == 0){
				ele.parent().layout('expand','west');
			}else{
				ele.parent().layout('collapse','west');
			}
			
		});
		$(this).parent().append(btn);
	});
	$('.panel.layout-panel.layout-panel-east.panel-htop.layout-split-east').each(function(){//east panel
		
		var centerPanel = $(this).parent().children('.panel.layout-panel.layout-panel-center.panel-htop');
		
		var _right = centerPanel.css('right');
		var _top = centerPanel.css('top');
		var _bottom = centerPanel.css('bottom');
		var _height = centerPanel.css('height');
		var ab_top = parseInt(_top)+parseInt(_height)/2;
		...
		var btn = $("<p class='layout_toolbar_lr' style='cursor:pointer;z-index:20;position:absolute;top:"+ab_top+";right:"+ab_right+"' )}></p>");
		
		btn.click(function(){
			if(parseInt(_right) == 0){
				
			}else{
				ele.parent().layout('collapse','east');
			}
			
		});
		$(this).parent().append(btn);
	});
//	$('.panel.layout-panel.layout-panel-north.panel-htop.layout-split-north').each(function(){//north panel
//		
//		var centerPanel = $(this).parent().children('.panel.layout-panel.layout-panel-center.panel-htop');
//		
//		var _top = centerPanel.css('top');
//		var _width = centerPanel.css('width');
//		var _left = centerPanel.css('left');
//		var leftPanel = $(this).parent().children('.panel.layout-panel.layout-panel-west.panel-htop.layout-split-west');
//		
//		var ab_top = _top;
//		var ab_left; 
//		if(leftPanel.length != 0 && parseInt(_left) != 0){
//			ab_left = parseInt(_width)/2+parseInt(leftPanel.css('width'));
//		}else{
//			ab_left = parseInt(_width)/2;
//		}
//		
//		var ele = $(this);
//		var btn = $("<p class='layout_toolbar_tb' style='cursor:pointer;z-index:30;position:absolute;top:"+ab_top+";left:"+ab_left+"' )}></p>");
//		btn.click(function(){
//			if(parseInt(ab_top) == 0){
//				ele.parent().layout('expand','north');
//			}else{
//				ele.parent().layout('collapse','north');
//			}
//			
//		});
//		$(this).parent().append(btn);
//	});
	$('.panel.layout-panel.layout-panel-south.panel-htop.layout-split-south').each(function(){//south panel
		var centerPanel = $(this).parent().children('.panel.layout-panel.layout-panel-center.panel-htop');
		var southHeight=$('.layout-split-south').height();
		//console.log(hh);
		var _bottom = centerPanel.css('bottom');
		if (_bottom=="auto") {
			if (southHeight==28) {
		...
		var leftPanel = $(this).parent().children('.panel.layout-panel.layout-panel-west.panel-htop.layout-split-west');
		var moveSize = 8;
		var ab_bottom = parseInt(_bottom)-moveSize;
		if(parseInt(_bottom) < 20){
			ab_bottom = -8;
		}
		var ab_left; 
		if(leftPanel.length != 0 && parseInt(_left) != 0){
		...
		var ele = $(this);
		var btn;
		if(parseInt(ab_bottom)+moveSize <= 0){
			btn = $("<p class='layout_toolbar_tb_close layout_toolbar' style='cursor:pointer;z-index:30;position:absolute;bottom:"+ab_bottom+";left:"+ab_left+"' )}></p>");
		}else{
			btn = $("<p class='layout_toolbar_tb_open layout_toolbar' style='cursor:pointer;z-index:30;position:absolute;bottom:"+ab_bottom+";left:"+ab_left+"' )}></p>");
		}
		
		btn.click(function(){
			if(parseInt(ab_bottom)+moveSize <= 0){
				ele.parent().layout('expand','south');
			}else{
				ele.parent().layout('collapse','south');
			}
		});
		$(this).parent().append(btn);
	});
